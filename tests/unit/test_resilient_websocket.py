"""Tests for swarm_common.resilient_websocket — ResilientWebSocket.

Tests the shared WebSocket resilience layer: connection lifecycle,
send/recv, heartbeat strategy, backoff calculation, and disconnect detection.
"""
...
def _mock_ws_connect(mock_ws: AsyncMock | None = None) -> AsyncMock:
    """Create an AsyncMock for websockets_connect that returns a mock WebSocket.

    websockets.connect() is awaitable — so the mock must return an awaitable
    that resolves to the mock WebSocket.
    """
    if mock_ws is None:
        mock_ws = AsyncMock()
...
class TestConnectionState:
    ...
    def test_values(self) -> None:
        """Verify all states exist with expected string values."""
        assert ConnectionState.DISCONNECTED == "DISCONNECTED"
        assert ConnectionState.CONNECTING == "CONNECTING"
        assert ConnectionState.CONNECTED == "CONNECTED"

    def test_is_str_enum(self) -> None:
        """ConnectionState values are strings."""
...
class TestAppLevelHeartbeat:
    ...
    def test_intercept_recv_recognizes_pong(self) -> None:
        """intercept_recv returns True for matching pong payload."""
    ...
    def test_intercept_recv_resets_timer(self) -> None:
        """Receiving a pong resets the last_pong_time."""
        hb = AppLevelHeartbeat(
            ping_payload=b"ping", pong_payload=b"pong",
        )
    ...
    async def test_run_detects_timeout(self) -> None:
        ...
        hb = AppLevelHeartbeat(
            ping_payload=b"ping", pong_payload=b"pong",
            interval_s=0.05, timeout_s=0.01,  # timeout < interval → immediate detection
        )
        # Set last_pong_time far in the past
    ...
    async def test_run_exits_on_send_failure(self) -> None:
        ...
        # Should exit cleanly without calling mark_dead
        mark_dead.assert_not_awaited()


# --- ResilientWebSocket Init ------------------------------------------------
...
class TestResilientWebSocketInit:
    ...
    def test_default_state(self) -> None:
        """New instance starts DISCONNECTED with event unset."""
...
class TestResilientWebSocketConnect:
    ...
    async def test_connect_failure_raises_connection_error(self) -> None:
        ...
        with patch(_WS_CONNECT, AsyncMock(side_effect=OSError("Connection refused"))):
    ...
    async def test_connect_clears_disconnected_event(self) -> None:
        """The disconnected_event is cleared on successful connect."""
        rws = ResilientWebSocket(url="ws://localhost")
...
class TestResilientWebSocketSendRecv:
    ...
    async def test_recv_intercepts_pongs_with_heartbeat(self) -> None:
        """recv_raw intercepts pong payloads when heartbeat is configured."""
        hb = AppLevelHeartbeat(
            ping_payload=b"ping", pong_payload=b"pong",
        )
        mock_ws = AsyncMock()
        # First recv returns pong (intercepted), second returns real data
    ...
    async def test_recv_passes_through_without_heartbeat(self) -> None:
        ...
        data = await rws.recv_raw()
        assert data == b"pong"  # Not intercepted
...
class TestResilientWebSocketDisconnect:
    ...
    async def test_disconnect_sets_event(self) -> None:
        """disconnect sets the disconnected_event."""
        rws = ResilientWebSocket(url="ws://localhost")
    ...
    async def test_mark_disconnected_fires_once(self) -> None:
        ...
        await rws._mark_disconnected()  # second call is no-op
        callback.assert_awaited_once()
...
class TestResilientWebSocketBackoff:
    ...
    def test_first_attempt_is_base(self) -> None:
        ...
        rws = ResilientWebSocket(
            ...
            backoff_base_s=1.0,
            backoff_max_s=60.0,
            backoff_jitter_factor=0.0,  # no jitter for deterministic test
        )
    ...
    def test_jitter_within_range(self) -> None:
        ...
        for _ in range(100):
            delay = rws.calc_backoff(0)
            assert 10.0 <= delay <= 12.5  # base + up to 25% jitter
...
class TestRawTransportCompliance:
    ...
    def test_satisfies_protocol(self) -> None:
        ...
        from swarm_common.channel import RawTransport, ReliableChannel

        rws = ResilientWebSocket(url="ws://localhost")
        # This should not raise — ResilientWebSocket structurally matches RawTransport
